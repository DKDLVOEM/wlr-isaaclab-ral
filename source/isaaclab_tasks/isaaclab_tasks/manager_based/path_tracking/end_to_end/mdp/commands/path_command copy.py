...
class PathCommand(CommandTerm):
    ...
    # NEW add: aggressiveness scalar g ( full changed )
    def _update_command(self):
        ...
        distances = torch.norm(path[:, :, :2] - root_pos.unsqueeze(1), dim=2)  # (num_envs, total_num)
        ...
        # Set the goal orientation (world frame yaw at the end of the path)
        path[env_indices, last_unique_indices, 2] = (
            self.full_path[env_indices, last_unique_indices, 2] + self.goal_yaw.unsqueeze(1)
        )
