...
def joint_pos_error(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    """The joint position errors of the asset.

    env.action_manager.action: [num_envs, num_actions]
      = [joint_actions(16), g(1)] 구조이므로,
    joint_pos_error 계산에서 g(마지막 1차원)는 제외하고 조인트 부분만 사용.
    """
    actions_all = env.action_manager.action          # (num_envs, 17)
    joint_actions = actions_all[:, :-1]              # (num_envs, 16)  → g 제거

    return joint_actions[:, asset_cfg.joint_ids] - joint_pos_rel(env, asset_cfg)



def joint_vel_error(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    """The joint velocity errors of the asset.

    env.action_manager.action: [num_envs, num_actions]
      = [joint_actions(16), g(1)] 구조이므로,
    joint_vel_error 계산에서 g(마지막 1차원)는 제외하고 조인트 부분만 사용.
    """
    actions_all = env.action_manager.action          # (num_envs, 17)
    joint_actions = actions_all[:, :-1]              # (num_envs, 16)  → g 제거

    return joint_actions[:, asset_cfg.joint_ids] - joint_vel_rel(env, asset_cfg)
...
# NEW add waypoint
# def waypoints_list(env: ManagerBasedRLEnv, num_waypoints=20, command_name: str = "path_command") -> torch.Tensor:
...
# NEW add waypoint
# def waypoints_list_fixed(env: ManagerBasedRLEnv, num_waypoints=20, command_name: str = "path_command") -> torch.Tensor:
