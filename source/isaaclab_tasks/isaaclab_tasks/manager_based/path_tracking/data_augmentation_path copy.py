...
# NEW add drift
USE_WHEEL = True
USE_GO1   = False       # Unitree GO1 쓰면 True (ANYmal이면 False)
...
def _switch_legs_lr(dof, fixed_values=False):
    ...
    if USE_GO1:
        IDX_LEG_LF = [0, 4,  8]   # FL -> LF
        IDX_LEG_LH = [2, 6, 10]   # RL -> LH
        IDX_LEG_RF = [1, 5,  9]   # FR -> RF
        IDX_LEG_RH = [3, 7, 11]   # RR -> RH
    else:
...
def _transform_obs_left_right(obs, scan: bool):
    """Apply left-right symmetry for the reduced student observation layout."""
    ...
    # waypoint list (fixed, 10 waypoints * 3 values)
    dim = 3
    transform = torch.tensor([1, -1, -1], device=device)
    # NEW add waypoint
    # for _ in range(20):
    ...
    # assert idx == num_obs == STUDENT_POLICY_OBS_DIM, f"Expected {STUDENT_POLICY_OBS_DIM} but got {idx}"
    assert idx == num_obs, f"Expected {num_obs} but got {idx}"
...
def _transform_obs_front_back(obs, scan: bool):
    """Apply front-back symmetry for the reduced student observation layout."""
