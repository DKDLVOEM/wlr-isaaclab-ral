...
# NEW add: aggressiveness scalar g
@configclass
class AggressivenessActionCfg(ActionTermCfg):
    """Configuration for scalar aggressiveness factor g.

    This reads a 1-D action from the policy and stores it into the env
    (e.g., env.ayro_g) for the AYRO module.
    """

    class_type: type[ActionTerm] = agg_actions.AggressivenessAction

    scale: float = 1.0
    """Scale factor for the raw action ([-1,1])."""
